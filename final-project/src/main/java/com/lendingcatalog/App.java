...
public class App {
    ...
    private final File dataFile = new File(FILE_BASE_PATH + "members.dat");
    ...
    private void initialize() {
        ...
        try (Scanner data = new Scanner(dataFile)){
            ...
            while (data.hasNextLine()){
                ...
                //create member
                String[] result = currentLine.split(FIELD_DELIMITER);
                Member currentMember = new Member(result[0], result[1]);
                File itemsFile = new File(FILE_BASE_PATH + result[2]);

                //Add member items
                try(Scanner memberData = new Scanner(itemsFile)) {

                    while (memberData.hasNextLine()){
                        String memberDataLine = memberData.nextLine();
                        String[] memberItems = memberDataLine.split(FIELD_DELIMITER);
                        // create book
                        if(memberItems[0].equals("book")){
                            Book book = new Book(memberItems[1], memberItems[2], LocalDate.parse(memberItems[3]));
                            book.registerItem();
                            items.add(book);
                        }
                        //create movie
                        else if(memberItems[0].equals("movie")){
                            Movie movie = new Movie(memberItems[1], memberItems[2], LocalDate.parse(memberItems[3]));
                            movie.registerItem();
                            items.add(movie);
                        }
                        //create tool
                        else if(memberItems[0].equals("tool")){
                            Tool tool = new Tool(memberItems[1], memberItems[2], Integer.parseInt(memberItems[3]));
                            tool.registerItem();
                            items.add(tool);

                        }
                    }
                    catalog.put(currentMember.toString(), items);
                }
            }
